...
if (!$conn->set_charset("utf8mb4")) {
    die("خطأ في ضبط الترميز: " . $conn->error);
}

// جلب الحجوزات المستقبلية لقسم البشرة
$today = date("Y-m-d");
$sql = "SELECT id, customer_name, phone, booking_date, booking_time, service, notes, image, booking_price 
        FROM bookings 
        WHERE category_id = 1 AND booking_date >= ? 
...
<head>
  <meta charset="UTF-8">
  <title>حجوزات قسم البشرة - Top Sisters</title>
  <style>
    body {
...
<body>
  <h2>🌸 حجوزات قسم البشرة</h2>
  <h2>
    <a href="facial_archive.php" class="archive-link">عرض الأرشيف</a>
  </h2>
